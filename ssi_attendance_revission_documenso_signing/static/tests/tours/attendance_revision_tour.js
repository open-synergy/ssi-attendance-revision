...
odoo.define(
    "ssi_attendance_revission_documenso_signing.attendance_revision_tour",
    function (require) {
        "use strict";
        ...
        // Flow). Navigation (open menu -> open record) is retraced from the
        // base IK ssi_attendance_revission/docs/attendance_revision/05-approve.md
        ...
        // True`; this tab is always present once this module is installed,
        // regardless of whether Documenso signing is actually used for the
        // approval (Modified Flow, bullet 1). The fixture's active Approval
        // Template has no Documenso Signing Template configured, so base
        // Flow steps 3-4 (Approve / OK) apply unchanged, exactly as
        // documented in ssi_attendance_revission/docs/attendance_revision/
        // 05-approve.md and in this module's own Modified Flow bullet 2 --
        // the tour therefore continues to completion (Done) without
        // contacting any external Documenso server.
        tour.register(
            ...
            [
                ...
                // ── Base Flow 2 — Open the record to approve.
                {
                    ...
                    trigger:
                        ".o_data_row:contains(TOUR AR DS Employee Approve) .o_data_cell:first",
                    extra_trigger: ".o_list_view",
                },
                {
                ...
                },
                {
                    ...
                    // `approval_signature_request_id` many2one widget
                    // itself -- a many2one rendered with no value has no
                    // text node inside its link, so it collapses to a
                    // zero-size box and jQuery's `:visible`
                    // (offsetWidth/offsetHeight) never matches it, hanging
                    // the tour until timeout. See skill
                ...
                // ── Post-Condition — all approval levels fulfilled: the
                // document is automatically finished, status jumps
                // straight to Done. This model has no Done button

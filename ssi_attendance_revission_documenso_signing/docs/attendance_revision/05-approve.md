...
> **Module:** ssi_attendance_revission_documenso_signing
>
> **Extends:** ssi_attendance_revission — model `attendance_revision`, aksi `05-approve`
...
- The behavior below only applies **when the record's active Approval Template has a
  ...
  `ssi_attendance_revission/docs/attendance_revision/05-approve.md`.
...
- Instead of clicking Approve, the user opens the linked request — shown under the
  ...
  **Signature Requests** tab (or via the **Open Signature Requests** button in the same
  ...
  Odoo, in Documenso.
- Approval completes automatically — without any button click on this Attendance
  Revision record — once the linked signature request reaches the **Signed** status
  (checked manually via **Check Status** on the request, or synced automatically by the
  connector). At that point, the base Post-Condition applies as usual: status changes
  automatically to **Done** — this model has no separate Finish/Done button.
- If the linked signature request is **cancelled** instead (e.g. a signer declines in
  ...
  Reject action (`ssi_attendance_revission/docs/attendance_revision/06-reject.md`), but
  triggered automatically by the cancelled request rather than by a user clicking
  Reject.
